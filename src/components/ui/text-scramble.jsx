...
export function TextScramble({ text, className = "" }) {
  ...
  return (
    ...
    >
      ...
      <span className="relative h-px w-full mt-1 overflow-hidden">
        <span
          className={`absolute inset-0 transition-transform duration-500 ease-out origin-left ${
            isHovering ? "scale-x-100 bg-current" : "scale-x-0"
          }`}
        />
      </span>
      ...
      <span
        className={`absolute -inset-3 rounded-lg transition-opacity duration-300 -z-10 ${
          isHovering ? "opacity-100 bg-current/5" : "opacity-0"
        }`}
      />
    </span>
  )
